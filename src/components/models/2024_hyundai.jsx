...
Command: npx gltfjsx@6.5.3 2024_hyundai.glb 
...
export function Hyundai_SUV(props) {
  const { nodes, materials } = useGLTF('/2024_hyundai.glb');

  const [targetColorIndex, setTargetColorIndex] = useState(0);

  // check materials
  // console.log('Hyundai model loaded: ', materials);

  // Get Materials
  const bodyMaterials = Object.values(materials).filter((mat) =>
  ...
  // bodymaterial roughness

  // Update target color ref when target color index change
  useEffect(() => {
    if (bodyMaterials) {
  ...
  }, [targetColorIndex, goodColors, bodyMaterials])

  // UseFrame for smooth color interpolation
...
useGLTF.preload('/2024_hyundai.glb')
